...
superwalk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  if (va % (SUPERPGSIZE) != 0)
    panic("superwalk: va not aligned");
  ...
  for (a = va; a < va + npages * PGSIZE; a += sz) {
    ...
    if ((a % SUPERPGSIZE) == 0) {
      ...
      if (spte == pte) {
        ...
        if ((*pte & PTE_V) == 0) {
          printf("va=%ld pte=%ld\n", a, *pte);
          panic("uvmunmap: not mapped superpage");
        }
        ...
        *pte = 0; // invalidate the superpage PTE.
        continue; // skip the rest of the loop for this superpage.
      }
    }
  ...
  for (i = 0; i < sz; i += szinc) {
    ...
    if ((i % SUPERPGSIZE) == 0) {
      ...
      if (spte == pte) {
        ...
        if ((*pte & PTE_V) == 0) {
          printf("i=%ld pte=%ld\n", i, *pte);
          panic("uvmcopy: not mapped superpage");
        }
...
#ifdef LAB_PGTBL
void
print_pgtbl(pagetable_t pagetable, uint64 va_prefix, int table_level)
{
  if (pagetable == 0) {
    return;
  }
  ...
  for (int i = 0; i < 512; i++) {
    ...
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0) {
      ...
      printf("0x%p: pte=0x%p, pa=0x%p\n", (void *)(va_prefix + (i << PGSHIFT)), (void *)pte,
             (void *)PTE2PA(pte));
    } else if (pte & PTE_V) {
...
{
  if (va % SUPERPGSIZE == 0) {
    // If the address is aligned to a superpage, use superwalk.
    pte_t *pte = superwalk(pagetable, va, 0);
